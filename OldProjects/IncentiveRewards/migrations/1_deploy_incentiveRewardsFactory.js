...
module.exports = async (deployer, network, accounts) => {
  ...
  */
  ...
  if (network == "development") {
    ...
    const myMarketsInstance = await deployProxy(Markets, [myMktHelperinstance.address, myPriceHelperInst.address], {
      from: tokenOwner
    });
    ...
    const myStakingRewardsFactoryInstance = await deployProxy(IncentiveRewardsFactory,
      [myRewardTokeninstance.address, genesisDate + 5, myMarketsInstance.address], {
        from: tokenOwner
      });
    console.log('myStakingRewardsFactoryInstance Deployed: ', myStakingRewardsFactoryInstance.address);

    await myMarketsInstance.setRewardsFactory(myStakingRewardsFactoryInstance.address, {
      from: tokenOwner
    })

    await myPriceHelperInst.setControllerAddress(myMarketsInstance.address, {from: tokenOwner})
